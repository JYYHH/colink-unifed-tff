...
from unifed.frameworks.tff.util import store_error, store_return, GetTempFileName, get_local_ip
...
def run_external_process_and_collect_result(cl: CL.CoLink, participant_id,  role: str, server_ip: str, Config: str):
...
def run_server(cl: CL.CoLink, param: bytes, participants: List[CL.Participant]):
    ...
    return run_external_process_and_collect_result(cl, participant_id, "server", server_ip, json.dumps(unifed_config))


@pop.handle("unifed.tff:client")
@store_error(UNIFED_TASK_DIR)
@store_return(UNIFED_TASK_DIR)
def run_client(cl: CL.CoLink, param: bytes, participants: List[CL.Participant]):
    ...
    # run external program
    participant_id = [i for i, p in enumerate(participants) if p.user_id == cl.get_user_id()][0]
    return run_external_process_and_collect_result(cl, participant_id, "client", server_ip, json.dumps(unifed_config))
